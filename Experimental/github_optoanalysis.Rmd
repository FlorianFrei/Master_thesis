...
A <- Plot_overview(Trialless %>% filter(phase == "cortex") %>% filter(between(rel_time,0,0.06)) %>% mutate(Behv = Behv2), orderby_Behv = "Airpuff")
...
B <- Plot_overview(Trialless %>% filter(phase == "Thalamus") %>% filter(between(rel_time,0,0.06)) %>% mutate(Behv = Behv2), orderby_Behv = "Airpuff")
...
print(pplot)
# graph2png(x= pplot, file='C:/Users/Freitag/Desktop/Opto',
