...
Data <- rbind(W2T,A2L,PC) %>% unite('unit', c(unit, Mouse,Day))  %>% filter(p_val < 0.05/3) %>% select(-X,-p_val)
```
...
Unit_Behv <- Data %>% left_join(Unit_behv) %>% rename(ZETA_Behv = Behv)

Unit_Behv %>% write_csv("C:/Users/deepl/Desktop/com_Data/wrangled_Data/Unit_Behv.csv")
```

```{r}
Unit_Behv %>% count(Behv_combined,sort=T) %>% mutate(perc = (n / sum(n))*100)
Data2 <- Data 
